...
// Provider: state들을 어떤 컴포넌트에서부터 제공할 것인가를 지정 (울타리로 감싸줌)
// Provider의 store 라는 prop을 울타리 안의 컴포넌트들에서 사용 가능함.
...
function ShowChart() {
    ...
    function reducer(currentState, action) { //인자: 현재 state값, 어떻게 바꿀 것인지에 대한 action 명세
    ...
    useEffect(() => {
        const asyncFetchData = createAsyncThunk(
            'Slice/asyncFetchData',
