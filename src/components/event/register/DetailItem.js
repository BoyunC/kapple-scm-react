...
const DetailItem = ({ componentName, componentData, updateComponentData, setUploadedFiles }) => {
	...
	const handleDefectRateChange = (e) => {
		...
		updateComponentData({ defective_rate: newDefectRate });
	};

	const handleQualityGradeChange = (e) => {
		const newQualityGrade = e.target.value;
		updateComponentData({ quality_grade: newQualityGrade });
	};

	const handleProductionPeriodChange = (e) => {
		const newProductionPeriod = e.target.value;
		updateComponentData({ prod_period: newProductionPeriod });
	};
				...
				<div className="row gx-3">
					...
					<div className="col-12 d-flex justify-content-between">
						<div className="w-50">
							...
							<input className="form-control mb-xl-3" type="text" placeholder="불량률" value={componentData.defective_rate} onChange={handleDefectRateChange} />
						</div>
						<div className="w-50 ms-3">
							<h5 className="text-1000">생산기간(일)</h5>
							<input className="form-control mb-xl-3" type="text" placeholder="생산기간" value={componentData.prod_period} onChange={handleProductionPeriodChange} />
						</div>
					</div>
					...
					<div className="col-12 mb-2">
						...
						<select className="form-select mb-3" aria-label="qualityGrade" value={componentData.quality_grade} onChange={handleQualityGradeChange}>
					...
					<FileUpload setUploadedFiles={setUploadedFiles} componentData={componentData} updateComponentData={updateComponentData} />
				</div>
			</div>
